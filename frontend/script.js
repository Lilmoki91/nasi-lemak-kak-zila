...
if (orderBtn) {
    if (!kedaiBuka()) {
        orderBtn.innerHTML = '<i class="fas fa-times" style="font-size:18px; margin-right:8px;"></i> ' + mesejTutup();
        orderBtn.style.background = '#EF4444';
        orderBtn.style.opacity = '0.7';
    } else {
        orderBtn.innerHTML = '<i class="fab fa-whatsapp" style="font-size:20px; margin-right:8px;"></i> Order WhatsApp';
...
function updateOwnerMemoUI() {
    ...
    // BUANG SEMUA INLINE STYLE - GUNA CLASS SAHAJA
    memoEl.style.background = '';
    memoEl.style.color = '';
    ...
    if (ownerMode === 'TUTUP') {
        ...
        memoEl.className = 'memo-status-tutup'; // CLASS UNTUK TUTUP
    } else if (ownerMode === 'BUKA') {
        memoEl.textContent = '🟢 ' + (currentMemo || 'Kedai dibuka khas!');
        memoEl.style.display = 'block';
        memoEl.className = 'memo-status-buka'; // CLASS UNTUK BUKA
    } else if (currentMemo) {
        memoEl.textContent = '📢 ' + currentMemo;
        memoEl.style.display = 'block';
        memoEl.className = 'memo-status-info'; // CLASS UNTUK INFO
    } else {
