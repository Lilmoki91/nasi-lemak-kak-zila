...
let troli = {};
let ownerMode = 'AUTO';
let operatingHours = { buka: '19:30', tutup: '00:00', hari_tutup: [4] };
let adminToken = null;
...
// 📸 Image upload state
let selectedFile = null;        // File yang dipilih dari device
let finalImageUrl = '';         // URL akhir (sama ada dari upload atau URL manual)
let isUploading = false;        // Flag semasa upload
...
// ==============================================
// 📸 IMAGE COMPRESSION & UPLOAD FUNCTIONS
// ==============================================

// Compress gambar guna canvas
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = function(e) {
            ...
            img.onload = function() {
                ...
                // Convert ke blob (JPEG untuk compress lebih baik)
                canvas.toBlob(
                    (blob) => {
                        if (blob) {
                            resolve(blob);
...
async function uploadImageToStorage(file, menuId) {
    ...
    const ext = 'jpg'; // Sentiasa save sebagai JPEG lepas compress
    const fileName = menuId 
        ? `menu/${menuId}_${timestamp}.${ext}`
        : `menu/new_${timestamp}.${ext}`;
    
    const storageRef = window.fbRef(storage, fileName);
    
    // Compress dulu
    const compressedBlob = await compressImage(file);
    
    // Upload
    const snapshot = await window.fbUploadBytes(storageRef, compressedBlob, {
...
// Padam gambar dari Firebase Storage
async function deleteImageFromStorage(imagePath) {
    if (!imagePath) return;
    
    // Hanya padam kalau gambar dari Firebase Storage (bukan URL luar)
    if (!imagePath.includes('firebasestorage.app') && !imagePath.includes('firebase')) {
        console.log('⏭️ Gambar dari URL luar, tidak dipadam dari storage');
        return;
    }
    ...
    try {
        await waitForFirebase();
        const storage = window.firebaseStorage;
        const ref = window.fbRef(storage, imagePath);
        await window.fbDeleteObject(ref);
        console.log('✅ Gambar lama dipadam:', imagePath);
    } catch(e) {
        console.log('⚠️ Gagal padam gambar lama:', e.message);
        // Jangan block flow kalau gagal padam
    }
}
...
function handleFileSelect(event) {
    ...
    selectedFile = file;
    finalImageUrl = ''; // Reset URL kalau pilih file baru
...
function loadUrlImage() {
    ...
    if (!url) {
        toast('❌ Sila masukkan URL gambar');
        return;
    }
    ...
    testImg.onerror = function() {
        toast('❌ Gagal load gambar dari URL ini');
    };
    testImg.src = url;
}
...
// ==============================================
// ⏰ WAKTU OPERASI - FETCH FROM FIREBASE
// ==============================================
    ...
    try {
        ...
        if (snap.exists()) {
            ...
            operatingHours = {
                buka: d.buka || '19:30',
                tutup: d.tutup || '00:00',
                hari_tutup: d.hari_tutup || [4]
            };
        }
    } catch(e) {
        console.log('Gagal fetch waktu operasi, guna default');
    }
    updateWaktuDisplay();
}
...
function updateWaktuDisplay() {
    ...
    if (headerHari) {
        if (operatingHours.hari_tutup.length > 0) {
    ...
    if (lokasiWaktu) {
        let html = '';
        for (let i = 0; i < 7; i++) {
            const isTutup = operatingHours.hari_tutup.includes(i);
...
function isHariTutup() {
    ...
    return operatingHours.hari_tutup.includes(today);
}
...
function mesejTutup() {
    ...
    if (!isWaktuOperasi()) return `🕐 Buka: ${formatTime12(operatingHours.buka)} - ${formatTime12(operatingHours.tutup)}`;
    return '';
}

// ==============================================
// 🍽️ FETCH MENU FROM FIREBASE
// ==============================================
async function fetchMenu() {
    try {
        ...
        if (menuData.length === 0) {
            menuData = [
                { id: '1', nama: 'Nasi Lemak Berlauk', desc: 'Ayam goreng, telur, sambal, ikan bilis', harga: 5.00, gambar: '', gambarPath: '', featured: true },
                { id: '2', nama: 'Nasi Lemak Biasa', desc: 'Nasi lemak klasik dengan sambal', harga: 2.00, gambar: '', gambarPath: '', featured: false },
                { id: '3', nama: 'Karipap', desc: 'Karipap rangup & sedap', harga: 1.00, gambar: '', gambarPath: '', featured: false },
                { id: '4', nama: 'Air Balang', desc: 'Minuman segar menyegarkan', harga: 1.00, gambar: '', gambarPath: '', featured: false }
            ];
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    // Semua kod yang guna DOM element — letak sini
    const chatBody = document.getElementById('sitiChatBody');
    const scrollBtn = document.getElementById('scrollBottomBtn');
    
    if (chatBody) {
        chatBody.addEventListener('scroll', function() {
            if (chatBody.scrollHeight - chatBody.scrollTop - chatBody.clientHeight > 200) {
...
async function loadCurrentSettings() {
    try {
        const res = await fetch('https://zila-food.onrender.com/api/admin/settings');
        const data = await res.json();
        currentMode = data.mode || 'AUTO';
        currentMemo = data.memo || '';
        setMode(currentMode);
        document.getElementById('adminMemo').value = currentMemo;
        document.getElementById('infoUpdate').textContent = data.last_updated ? new Date(data.last_updated).toLocaleString('ms-MY') : 'Belum pernah';
        if (currentMemo) { document.getElementById('memoPreview').style.display = 'block'; document.getElementById('memoPreviewText').textContent = currentMemo; }
        document.getElementById('infoWaktu').textContent = `${formatTime12(operatingHours.buka)} - ${formatTime12(operatingHours.tutup)}`;
    } catch(e) { document.getElementById('infoUpdate').textContent = 'Gagal load'; }
}
...
async function simpanSettings() {
    ...
    try {
        const res = await fetch('https://zila-food.onrender.com/api/admin/settings', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token: adminToken, mode: currentMode, memo })
        });
        const data = await res.json();
        if (data.success) {
            msg.textContent = '✅ Tetapan disimpan!'; msg.style.color = '#2E7D32';
            document.getElementById('infoUpdate').textContent = new Date().toLocaleString('ms-MY');
            if (memo) { document.getElementById('memoPreview').style.display = 'block'; document.getElementById('memoPreviewText').textContent = memo; }
            setTimeout(() => { msg.style.display = 'none'; toast('✅ Tetapan dikemaskini!'); }, 1500);
            fetchOwnerSettings();
            kemasKiniUI();
        } else { msg.textContent = data.message || '❌ Gagal simpan.'; msg.style.color = '#EF4444'; }
    } catch(e) { msg.textContent = '❌ Gagal sambung ke server.'; msg.style.color = '#EF4444'; }
}
...
async function editMenu(docId) {
    try {
        ...
        // Kalau ada gambar existing, tunjuk preview
        if (m.gambar) {
...
async function simpanMenu() {
    if (isUploading) {
        toast('⏳ Sedang memuat naik gambar...');
        return;
    }
    ...
    try {
        ...
        // 📸 SCENARIO 1: User pilih file baru dari device → UPLOAD
        if (selectedFile) {
            isUploading = true;
            showUploadProgress(10, 'Memampatkan gambar...');
            
            try {
                // Generate temporary ID untuk filename kalau menu baru
                ...
                // Kalau edit menu & ada gambar lama → padam
                if (editId && existingGambarPath) {
                    needDeleteOld = true;
                }
        ...
        // 📸 SCENARIO 2: User guna URL manual
        else if (finalImageUrl) {
            gambarURL = finalImageUrl;
            gambarPath = ''; // URL luar, tiada path
            
            // Kalau edit & ada gambar lama dari Firebase → padam
            if (editId && existingGambarPath) {
                needDeleteOld = true;
            }
        }
        // 📸 SCENARIO 3: User tak tukar gambar (edit mode) → keep existing
        else if (editId) {
        ...
        if (editId) {
            ...
            // 📸 Kalau menu baru & ada upload, rename file dengan ID yang betul
            if (selectedFile && gambarPath) {
                try {
                    // Upload semula dengan nama yang betul (guna ID sebenar)
        ...
        // Padam gambar lama (async, jangan block)
        if (needDeleteOld && existingGambarPath) {
            deleteImageFromStorage(existingGambarPath); // fire-and-forget
        }
...
async function padamMenu(docId) {
    ...
    try {
        ...
        // Dapatkan gambar path dulu sebelum padam
        const docRef = window.fbDoc(db, 'menu', docId);
        const docSnap = await window.fbGetDoc(docRef);
        ...
        // Padam gambar dari Storage (fire-and-forget)
        if (gambarPath) {
            deleteImageFromStorage(gambarPath);
        }
...
// ==============================================
// 📋 FETCH OWNER SETTINGS (MEMO)
// ==============================================
async function fetchOwnerSettings(retryCount = 0) {
    try {
        const res = await fetch('https://zila-food.onrender.com/api/admin/settings');
        const data = await res.json();
        ...
        if (data.mode === 'TUTUP') {
            memoEl.textContent = '🔴 ' + (data.memo || 'Kedai ditutup buat sementara.');
            memoEl.style.display = 'block';
            memoEl.style.background = '#FEF2F2';
            memoEl.style.color = '#DC2626';
        } else if (data.mode === 'BUKA') {
            memoEl.textContent = '🟢 ' + (data.memo || 'Kedai dibuka khas!');
            memoEl.style.display = 'block';
            memoEl.style.background = '#F0FDF4';
            memoEl.style.color = '#16A34A';
        } else if (data.memo) {
            memoEl.textContent = 'ℹ️ ' + data.memo;
            memoEl.style.display = 'block';
        ...
        if (retryCount < 3) setTimeout(() => fetchOwnerSettings(retryCount + 1), 2000);
    }
}
...
async function initApp() {
    ...
    setInterval(async () => {
        await Promise.all([fetchOwnerSettings(), fetchWaktuOperasi()]);
        kemasKiniUI();
    }, 30000);
}
