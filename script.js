...
let isImageDeleted = false; // 🔥 TAMBAH INI: Untuk kesan user tekan butang X
...
// ==============================================
// 📸 UPLOAD GAMBAR KE BACKEND API (SIMPAN image_id)
// ==============================================
async function uploadImageToStorage(file, menuId) {
    ...
    try {
        ...
        if (data.success) {
            ...
            return {
                url: data.url,
                // 🔥 Simpan image_id, bukan delete_url!
                path: 'imgbb:' + data.image_id,
                size: data.size
            };
        } else {
...
// ==============================================
// 🗑️ PADAM GAMBAR MELALUI BACKEND API (GUNA image_id)
// ==============================================
async function deleteImageFromStorage(imagePath) {
    ...
    if (imagePath.startsWith('imgbb:')) {
        try {
            const imageId = imagePath.replace('imgbb:', '');
            ...
            if (response.ok && responseData.success) {
                console.log('✅ Gambar BERJAYA dipadam dari ImgBB!');
            } else {
    ...
    console.log('⏭️ URL luar atau bukan dari ImgBB, tidak dipadam:', imagePath);
}
...
function removeUploadPreview(event) {
    ...
    selectedFile = null;
    finalImageUrl = '';
    isImageDeleted = true; // 🔥 TAMBAH INI: Tanda user sengaja padam!
...
isImageDeleted = false; // 🔥 TAMBAH INI
}
...
async function simpanMenu() {
    ...
    try {
        ...
        let needDeleteOld = false;
                ...
                // 📸 LOGIC GAMBAR YANG BETUL
        if (selectedFile) {
            // SCENARIO 1: User upload gambar baru dari device
            isUploading = true;
            ...
            // SCENARIO 2: User guna URL luar baru
            gambarURL = finalImageUrl;
            gambarPath = 'url_luar:' + finalImageUrl; // Tanda ini URL luar
            
            // Padam gambar lama dari ImgBB
            if (editId && existingGambarPath && existingGambarPath.startsWith('imgbb:')) {
                needDeleteOld = true;
            }
            
        } else if (isImageDeleted) {
            // 🔥 SCENARIO 3: User tekan butang X (PADAM GAMBAR SEPENUHNYA)
            gambarURL = '';
            gambarPath = '';
            ...
            // SCENARIO 4: User tak sentuh gambar langsung (Kekalkan gambar lama)
