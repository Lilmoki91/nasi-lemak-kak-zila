...
async function editMenu(docId) {
    try {
        ...
        if (m.gambar) {
            ...
            // 🔥 FIX AUTO: Cari gambarPath. Kalau takde, extract dari URL Cloudinary
            ...
            if (!pathToSave && m.gambar.includes('res.cloudinary.com')) {
                try {
                    ...
                    if (urlParts.length > 1) {
                        ...
                        // Buang version (v123/) dan extension (.jpg)
                        const publicId = versionAndId.replace(/v\d+\//, '').replace(/\.[^/.]+$/, "");
                        pathToSave = 'cloudinary:' + publicId;
                        
                        // Auto-save balik ke Firestore supaya lepas ni tak perlu extract lagi
...
async function simpanMenu() {
    ...
    try {
        ...
        let needDeleteOld = false;
                        ...
                        // 📸 LOGIC GAMBAR YANG BETUL (FIXED FOR CLOUDINARY)
        if (selectedFile) {
            ...
            try {
                ...
                // 🔥 FIX: Padam gambar lama (sama ada cloudinary atau imgbb)
            ...
            // 🔥 FIX INI PUNCA MASALAH: Check cloudinary jugak!
            if (editId && existingGambarPath && (existingGambarPath.startsWith('cloudinary:') || existingGambarPath.startsWith('imgbb:'))) {
                needDeleteOld = true;
